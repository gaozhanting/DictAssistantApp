...
//  DictAssistantApp
//
//  Created by Gao Cong on 2021/10/2.
//

import Foundation

// refer to https://github.com/RockfordWei/GBig
...
private class GBig {
    ...
    static let siDict: Dictionary<Character, Int> = {
        var result: Dictionary<Character, Int> = Dictionary()
        for (i, s) in gb.enumerated() {
            result[s] = i
        }
        return result
    }()
    
    static let isDict: Dictionary<Int, Character> = {
        var result: Dictionary<Int, Character> = Dictionary()
        for (i, s) in gb.enumerated() {
            result[i] = s
        }
        return result
    ...
    static let tiDict: Dictionary<Character, Int> = {
        var result: Dictionary<Character, Int> = Dictionary()
        for (i, t) in big5.enumerated() {
            result[t] = i
        }
        return result
    }()
    
    static let itDict: Dictionary<Int, Character> = {
        var result: Dictionary<Int, Character> = Dictionary()
        for (i, t) in big5.enumerated() {
            result[i] = t
        }
    ...
    static func simplifyOpt(_ ch: String) -> String {
    ...
    static func traditionalizeOpt(_ ch: String) -> String {
        return String(ch.map { c in
            if let i = siDict[c] {
                if let r = itDict[i] {
                    return r
    ...
    // deprecated, which is O(n), too slow.
...
extension String {
    var big5: String {
        return GBig.traditionalizeOpt(self)
    }
    var gb: String {
        return GBig.simplifyOpt(self)
    }
}
